...
use crate::{assets::CharsetAsset, enemies::spawn_boss_1, player::spawn_player};
...
fn prepare_level(
    ...
    spawn_boss(&mut commands, &chaset, window, game_status.level);

    next_state.set(GameState::Playing);
}

fn spawn_boss(commands: &mut Commands, chaset: &CharsetAsset, window: &Window, level: i32) {
    if level == 1 {
        spawn_boss_1(commands, chaset, window);
    } else {
        spawn_boss_1(commands, chaset, window);
    }
}
