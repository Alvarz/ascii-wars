...
pub(super) fn plugin(app: &mut App) {
    ...
    app.insert_resource(Pattern5Timer {
        timer: Timer::from_seconds(0.0, TimerMode::Repeating),
    });
    ...
    app.add_systems(
        Update,
        boss_shoot_pattern_5.run_if(in_state(GameState::Playing)),
    );
...
fn boss_shoot_pattern_5(
    mut commands: Commands,
    bosses: Query<(Entity, &Transform, &Pool, &ShootPattern5), (With<Boss>, With<ShootPattern5>)>,
    time: Res<Time>,
    charset: Res<CharsetAsset>,
    mut pattern_timer: ResMut<Pattern5Timer>,
) {
    if pattern_timer.timer.tick(time.delta()).just_finished() {
        for (e, transform, pool, pattern) in &bosses {
            pattern_timer.timer = Timer::from_seconds(pattern.fire_rate, TimerMode::Repeating);
...
fn the_hoarder(commands: &mut Commands, e: Entity) {
    commands.entity(e).insert((
        ShootPattern1 {
            bullet_speed: 80.,
            spawn_count: 64,
...
fn the_clockwork_leviathan(commands: &mut Commands, e: Entity) {
    commands.entity(e).insert((
        ShootPattern1 {
            bullet_speed: 250.,
            spawn_count: 12,
...
fn the_chaos_reaper(commands: &mut Commands, e: Entity) {
    commands.entity(e).insert((
        ShootPattern1 {
            bullet_speed: 200.,
            spawn_count: 6,
